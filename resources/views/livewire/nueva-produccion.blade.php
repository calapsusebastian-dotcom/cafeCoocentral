<x-slot:headerActions>
    <a href="{{ route('produccion.index') }}" class="hidden md:inline-flex items-center px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors">
        Cancelar
    </a>
    <div class="hidden md:flex flex-col items-end leading-tight px-3 py-2 rounded-xl bg-gray-50">
        <span class="text-[11px] text-gray-400">Producción #</span>
...
<div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
        <div class="xl:col-span-2 space-y-6">
            {{-- 1. Información de la producción --}}
            <section class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-6">
                <h2 class="text-sm font-semibold text-gray-900 mb-4">1. Información de la producción</h2>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label class="block">
                        <span class="text-xs text-gray-400">Fecha de producción *</span>
                        <input type="date" wire:model="fecha_produccion" class="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" />
                        @error('fecha_produccion') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </label>
                    <label class="block">
                        <span class="text-xs text-gray-400">No. producción</span>
                        <input type="text" wire:model="numero" placeholder="PROD-0001" class="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" />
            ...
            {{-- 3. Detalle de la producción --}}
            <section class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-6">
                <h2 class="text-sm font-semibold text-gray-900 mb-4">3. Detalle de la producción</h2>

                @if (empty($cart))
                    <p class="text-sm text-gray-400 py-8 text-center">Agrega los productos producidos hoy para verlos aquí.</p>
                @else
                                        ...
                                        <td class="px-1.5 py-3">
                                            <div class="inline-flex items-center rounded-lg border border-gray-200">
                                                ...
                                                <input
                                                    ...
                                                    class="w-10 text-center text-sm border-0 bg-transparent focus:outline-none focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                                />
                ...
                @if (! empty($cart))
                    <button type="button" wire:click="vaciarProduccion" class="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-200 text-sm text-gray-500 hover:border-red-200 hover:text-red-500 transition-colors">
                        <x-heroicon-o-trash class="w-4 h-4" />
                        Vaciar producción
                    </button>
                @endif
        ...
        <div class="xl:col-span-1">
            <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-6 xl:sticky xl:top-28">
                <h2 class="text-sm font-semibold text-gray-900 mb-4">Resumen de la producción</h2>
                ...
                <button
                    type="button"
                    wire:click="guardarProduccion"
                    wire:loading.attr="disabled"
                    wire:target="guardarProduccion"
                ...
                >
                    ...
                    <span wire:loading.remove wire:target="guardarProduccion">Guardar producción</span>
                    <span wire:loading wire:target="guardarProduccion">Guardando...</span>
                </button>

                <div class="mt-4 rounded-xl bg-amber-50 p-4">
                    <p class="text-xs font-medium text-amber-800 mb-2">Notas de la producción</p>
                    <textarea
                        wire:model="notas"
                        rows="3"
                        placeholder="Escribe aquí alguna nota especial para esta producción..."
