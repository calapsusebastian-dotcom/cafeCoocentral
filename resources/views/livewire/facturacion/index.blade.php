<div>
    ...
    <div class="flex gap-2 mb-5">
        <button
            type="button"
            wire:click="cambiarDestino('local')"
            @class([
                'flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors',
                'bg-brand-600 text-white shadow-sm shadow-brand-200' => $destino === 'local',
                'bg-white text-gray-500 ring-1 ring-gray-100 hover:text-gray-700' => $destino !== 'local',
            ])
        >
        ...
        <button
            type="button"
            wire:click="cambiarDestino('web')"
            @class([
                'flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors',
                'bg-brand-600 text-white shadow-sm shadow-brand-200' => $destino === 'web',
                'bg-white text-gray-500 ring-1 ring-gray-100 hover:text-gray-700' => $destino !== 'web',
            ])
        >
            ...
            <table class="w-full text-sm min-w-[700px]">
                <thead>
                    <tr class="text-left text-xs text-gray-400 border-b border-gray-100">
                        ...
                        <th class="px-2 py-2 font-medium">Factura</th>
                    ...
                    @forelse ($this->pedidos as $pedido)
                        <tr class="border-b border-gray-50 last:border-0">
                            ...
                            <td class="px-2 py-3 text-right space-x-2">
                                <button type="button" wire:click="verPedido({{ $pedido->id }})" title="Ver información completa" class="text-gray-400 hover:text-brand-600">
                                    <x-heroicon-o-eye class="w-4 h-4 inline" />
                                </button>
                                @unless ($pedido->status === 'cancelado')
                                    <button type="button" wire:click="abrirFactura({{ $pedido->id }})" title="{{ $pedido->numero_factura ? 'Editar factura' : 'Facturar pedido' }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-brand-600 hover:bg-brand-50">
                                        <x-heroicon-o-document-text class="w-4 h-4" />
                                        {{ $pedido->numero_factura ? 'Editar' : 'Facturar' }}
                                    </button>
                                @endunless
                            </td>
                        </tr>
